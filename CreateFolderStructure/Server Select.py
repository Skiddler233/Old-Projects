...
# Function: Checks the length of the folder name is less than 254 char to prevent errors
...
#         super(InitialFrame, self).__init__(*args, **kw)
#
#         # Adding components to Initial Frame
...
#
#         # Header
#         header_label = wx.StaticText(panel, label="Select Server")
...
#
#         # Client Type: Label and Dropdown
#         client_label = wx.StaticText(panel, label="Client Type:")
#         client_label.SetFont(modern_font_small)
#         vbox.Add(client_label, flag=wx.LEFT | wx.RIGHT | wx.TOP, border=10)

    #     server_choices = ["woolworths", "Make"]
    #     self.server_choice = wx.ComboBox(panel, choices=server_choices, style=wx.CB_READONLY)
    #     self.server_choice.SetFont(modern_font_small)
    #     vbox.Add(self.server_choice, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)
    #
    #     Continue Button
    #     continue_button = wx.Button(panel, label="Continue")
    #     continue_button.SetFont(modern_font_small)
    #     continue_button.Bind(wx.EVT_BUTTON, self.on_continue)
    #     vbox.Add(continue_button, flag=wx.LEFT | wx.ALIGN_RIGHT, border=10)
    #
    #     panel.SetSizer(vbox)
    #
    # def on_continue(self, event):
    #    selected_server = self.server_choice.GetValue()
    #    self.Hide()  # Hide the initial frame
    #    app_frame = AppFrame(None, title="CFS", size=(400, 715), server=selected_server)
    #    app_frame.Show()
    #
...
class AppFrame(wx.Frame):
    ...
    def init_ui(self):
        ...
        server_label = wx.StaticText(panel, label=f"Selected Server: {self.server}")
        server_label.SetFont(modern_font_small)
        vbox.Add(server_label, flag=wx.LEFT | wx.RIGHT | wx.TOP, border=10)
        ...
        def load_client_items(self):
            with open("data.json", "r") as file:
                data = json.load(file)
                return data["clients"]

        clients = load_client_items(self)
...
# Function
    ...
    def illegal_char(self):
        ...
        for char in job_number and description:
            if char in illegal_characters:
                found.append(char)
        if len(found) > 0:
            wx.MessageBox(f"Illegal character/s found: {found}", "Error", wx.OK | wx.ICON_ERROR)
            return False
        return True
    ...
    def on_submit(self, event):
        ...
        live_path = os.path.join(f"{self.server}/Clients/", folder_path)
...
if __name__ == '__main__':
    app = wx.App(False)
    initial_frame = AppFrame(None, title="Client Type Selection", size=(400, 700))
    initial_frame.Show()
    app.MainLoop()
