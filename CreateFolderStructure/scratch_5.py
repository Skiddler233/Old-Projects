...
class InitialFrame(wx.Frame):
    ...
    def init_ui(self):
        ...
        # Header
        header_label = wx.StaticText(panel, label="Select Client Type")
        ...
        client_choices = ["Client A", "Client B"]  # Update with your actual client types
        self.client_choice = wx.ComboBox(panel, choices=client_choices, style=wx.CB_READONLY)
        self.client_choice.SetFont(modern_font_small)
        vbox.Add(self.client_choice, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)
    ...
    def on_continue(self, event):
        selected_client_type = self.client_choice.GetValue()
        self.Hide()  # Hide the initial frame
        app_frame = AppFrame(None, title="CFS", size=(400, 700), client_type=selected_client_type)
        app_frame.Show()


class AppFrame(wx.Frame):
    def __init__(self, *args, **kw):
        client_type = kw.pop("client_type", None)  # Remove 'client_type' from kw
...
if __name__ == '__main__':
    app = wx.App(False)
    initial_frame = InitialFrame(None, title="Client Type Selection", size=(300, 150))
    initial_frame.Show()
    app.MainLoop()
