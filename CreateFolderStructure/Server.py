...
#def load_client_items(server):
#    server = server.lower()
#    with open("data.json", "r") as file:
#        data = json.load(file)
#        return data[f"{server}_clients"]
...
class AppFrame(wx.Frame):
    ...
    def init_ui(self):
        ...
        servers = load_server()
        self.server_dropdown = wx.ComboBox(panel, choices=servers, value="Make", style=wx.CB_READONLY)
        self.server_dropdown.SetFont(modern_font_small)
        vbox.Add(self.server_dropdown, flag=wx.EXPAND | wx.LEFT | wx.RIGHT, border=10)

        self.server_dropdown.Bind(wx.EVT_CHOICE, self.load_client_items(self.server_dropdown.GetValue()))
        ...
        self.client_dropdown.Bind(wx.EVT_COMBOBOX, self.business_unit_enable, self.load_client_items)
    ...
    def load_client_items(self, server):
        server = server.lower()
        with open("data.json", "r") as file:
            data = json.load(file)
            clients = data[f"{server}_clients"]
        self.client_dropdown.SetItems(clients)

    def server_selected(self):
        server = self.server_dropdown.GetValue()
        return server
...
# Function
    ...
    def on_submit(self, event):
        ...
        if not check_server_empty(server):
            return
        ...
        live_path = os.path.join(f"{server}/Clients/", folder_path)
